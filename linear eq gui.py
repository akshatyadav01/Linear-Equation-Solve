...
def maxm(x,y,z,i):
    if abs(x[i])>abs(y[i]) and abs(x[i])>abs(z[i]):
        return x
    elif abs(y[i])>abs(x[i]) and abs(y[i])>abs(z[i]):
        return y
    else:
        return z
